...
function PaymentStep({ data, onBack, onSubmit }: PaymentStepProps) {
  ...
  useEffect(() => {
    const fetchExchangeRate = async () => {
      try {
        ...
        const usdToPkr = 1 / data.rates.USD
        setPkrRate(usdToPkr)
        setLoadingRate(false)
      } catch (error) {
        console.error("[v0] Failed to fetch PKR exchange rate:", error)
        setLoadingRate(false)
      }
    }
  ...
  const totalAmountPKR = pkrRate ? (totalAmount * pkrRate).toFixed(2) : null
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-4xl mx-auto space-y-8 pb-8">
      <div className="space-y-6">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
            ...
            <Card className="p-6 sm:p-8 bg-white/40 backdrop-blur-md border-glass-border shadow-glow rounded-2xl space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  {loadingRate ? (
                    <div className="flex items-center justify-between py-2 text-sm">
                      <span className="text-muted">Converting to PKR...</span>
                    </div>
                  ) : pkrRate && totalAmountPKR ? (
                    <div className="flex items-center justify-between py-2 bg-primary/5 px-4 rounded-lg">
                      <span className="text-sm font-medium text-primary">Amount in PKR</span>
                      <span className="text-lg font-bold text-primary">₨ {totalAmountPKR}</span>
                    </div>
                  ) : null}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted leading-relaxed">
                  If you can share a screenshot on WhatsApp to our representative, add your phone number and we'll
                  contact you
                </p>
              </div>
          ...
          <div className="space-y-6">{/* Additional content can be added here if needed */}</div>
        </div>
      </div>
    </form>
  )
