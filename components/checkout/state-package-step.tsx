        ...
        <div className="space-y-3">
          {ENTITY_TYPES.map((entity) => (
            <button
              ...
              className={`w-full text-left p-3 sm:p-4 md:p-5 rounded-lg border-2 transition-all overflow-hidden ${
            ...
            >
              <div className="flex items-start gap-2 sm:gap-3">
                <input
                  ...
                  className="w-4 h-4 sm:w-5 sm:h-5 text-[#ff0d13] accent-[#ff0d13] mt-0.5 flex-shrink-0 cursor-pointer"
                />
                <div className="flex-1 min-w-0 overflow-hidden">
                  ...
                  <ul className="space-y-1 sm:space-y-1.5 mb-2 sm:mb-3">
                    {entity.features.map((feature, i) => (
                      <li key={i} className="text-xs sm:text-sm text-slate-700 flex items-start gap-1.5 sm:gap-2">
                        <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#ff0d13] flex-shrink-0 mt-0.5" />
                        <span className="break-words">{feature}</span>
                      </li>
                    ))}
                  </ul>
                  <p className="text-xs sm:text-sm text-slate-600 pt-2 sm:pt-3 border-t border-slate-100 break-words">
          ...
          {PACKAGES.map((pkg) => {
            ...
            return (
              <button
                ...
                className={`p-3 sm:p-4 md:p-5 rounded-lg border-2 cursor-pointer transition-all text-left flex flex-col overflow-hidden min-w-0 ${
              ...
              >
                <div className="flex items-start justify-between gap-2 sm:gap-3 mb-2 sm:mb-3">
                  <h3 className="text-base sm:text-lg font-semibold text-slate-900 break-words">{pkg.name}</h3>
                  {data.packageType === pkg.id && (
                ...
                <div className="mb-3 sm:mb-4">
                  <div className="text-xl sm:text-2xl md:text-3xl font-bold text-[#ff0d13]">${total}</div>
                  <div className="text-xs sm:text-sm text-slate-600 mt-0.5 sm:mt-1 break-words">
                ...
                <ul className="space-y-1.5 sm:space-y-2 flex-1">
                  {pkg.features.map((feature, i) => (
                    <li
                      key={i}
                      className={`text-xs sm:text-sm flex items-start gap-1.5 sm:gap-2 ${
