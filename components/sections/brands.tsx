    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden sm:grid grid-cols-5 gap-4 md:gap-6 lg:gap-8 justify-items-center items-center">
          {brands.map((b) => (
            <div key={b.name} className={`flex items-center justify-center ${b.name === 'nsave' ? 'ml-8' : ''}`}>
              <div className={`relative flex items-center justify-center h-[clamp(28px,6vw,72px)] ${b.name === 'nsave' ? 'w-[280px]' : 'max-w-[200px]'}`}>
                <img
                  src={b.logo}
                  alt={`${b.name} logo`}
