  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!user || user.role !== "admin") {
        console.log("[v0] User is not admin, redirecting to client dashboard")
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const activeCustomers = allUsers.filter((u: any) => u.role === "client" && u.status === "active").length
    ...
    <div className="space-y-8">
      ...
      <Card className="bg-white border-slate-200">
        <CardHeader>
          ...
          <p className="text-sm text-slate-600">Current month revenue from orders</p>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-3">
              {orders.length === 0 ? (
                ...
                orders.map((order) => (
                  <div
                    key={order.id}
                    className="flex items-center justify-between p-4 rounded-lg border border-slate-200 hover:border-primary/20 hover:shadow-md transition-all duration-200"
                  >
